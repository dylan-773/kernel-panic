...
- Two art generators: PixelLab (Pixel Artisan subscription, generations allowance resets monthly; `create_image_pro` at 256px with a shipped portrait as `style_image_url` matches the customer face set) and Higgsfield nano_banana_pro (2 credits/image). Customer faces ship full colour at native 256px; device macros, scans and stills use the 1-bit dither treatments (`pipeline/tools/dither.py`), pixel assets use `pxpost.py`.
- The shell is the pixel desktop (`vault/40-presentation/pixel-desktop.md`): `pixel-ui.css` is UNLAYERED and wins every property it sets over the `@layer legacy` design system in `styles.css`. A legacy `@container` block still applies where the pixel sheet is silent at that width; override it in `pixel-ui.css` with a matching container block, never by editing the legacy rule.
