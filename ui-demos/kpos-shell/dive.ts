...
import { play, playCascade, playStinger, startDrone, stopDrone, unlock } from "./sound";

/**
 * DIVE.EXE poster study (dive.html): the flood-claim duel rebuilt as a
 * full-screen instrument panel per the reference DNA - the board drawn as a
 * white-line circuit schematic (ref: black schematic poster), the frame laid
 * out like the ship-diagnostic OS (corner key rails, breadcrumb strip,
 * telemetry table, bottom console), turn state as boxed toggle pairs (ref:
 * fire-control checklist), the intrusion rendered as monochrome signal
 * glitch (ref: attack graph), never a second hue.
 *
...
function buildShell(): void {
  ...
  crumbRight.append(roundBox, R.daySlot, R.sndBtn);
  ...
  const turnPair = el("div", "dv-turnpair");
  R.turnYou = el("div", "dv-turncell", "YOU");
  R.turnOpp = el("div", "dv-turncell", "INTRUSION");
  turnPair.append(R.turnYou, R.turnOpp);
  railR.appendChild(turnPair);
  ...
  oppBox.appendChild(el("h3", "", "INTRUSION"));
  const rows = el("div", "kp-datarow-list");
  const r1 = datarow("RAM");
  R.oppRam = r1.em;
  const r2 = datarow("BANKED");
  R.oppBankRow = r2.row;
  R.oppBank = r2.em;
  const r3 = datarow("ARMED NODES");
  R.oppArmed = r3.em;
  rows.append(r1.row, r2.row, r3.row);
  ...
  const cellWrap = el("span", "kp-photo-cell-full");
  const img = new Image(880, 880);
  img.src = "art/dex-cramdeck.png";
  img.alt = "";
  cellWrap.appendChild(img);
  dev.appendChild(cellWrap);
...
function renderOverlay(): void {
  ...
  for (const [label, value, warn] of rows) {
    ...
    if (warn) row.classList.add("kp-datarow-warn");
  ...
  const again = el("button", "kp-btn2", "NEW DIVE");
  again.type = "button";
...
function render(): void {
  ...
  /* right rail */
  R.turnYou!.classList.toggle("dv-turn-on", state.turn === "player" && state.phase === "playing");
  R.turnOpp!.classList.toggle("dv-turn-on", state.turn === "opp" && state.phase === "playing");
  R.turnOpp!.classList.toggle("dv-turn-moving", state.turn === "opp" && state.phase === "playing");
  ...
  R.oppBankRow!.style.display = oppBanked > 0 ? "" : "none";
  R.oppBank!.textContent = `+${oppBanked} NEXT TURN`;
  ...
  const oppMoving = state.phase === "playing" && state.turn === "opp";
...
(function rig() {
  ...
  ["lavender", "magenta", "phosphor"].forEach((h, i) => {
    const b = el("button", i === 0 ? "rig-on" : "", h.toUpperCase());
    b.addEventListener("click", () => {
      hueRow.querySelectorAll("button").forEach((x) => (x.className = ""));
      b.className = "rig-on";
  ...
  re.addEventListener("click", () => boot(scenario, (Math.random() * 0xffffffff) >>> 0));
