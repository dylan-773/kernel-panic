...
`;
  ...
  async function loadSubmissions() {
    try {
      ...
      const next = m.demos.filter((d) => d.desktop && d.desktop.mountable).map((d) => ({
        id: d.id,
        title: d.title,
        src: `../${d.dir}/${d.entry}`,
        icon: d.desktop.icon,
  ...
  function runHooks(doc, hooks, first) {
...
PANIC`);
  ...
  function showDesktop(slotN) {
    ...
    const built = new Map;
    const winOf = (id) => mounted.get(id)?.win;
    const flow = buildShopfront(wFlow, () => winOf("loadout")?.open());
    buildManual(wManual);
    buildLedger(wLedger);
    ...
    const mountSub = (id) => {
      ...
      if (def.replaces)
        nativeIcons.get(def.replaces)?.style.setProperty("display", "none");
      repaintJobsBadge();
      return true;
    };
    const unmountSub = (id) => {
      ...
      mounted.delete(id);
      if (m.def.replaces)
        nativeIcons.get(m.def.replaces)?.style.removeProperty("display");
      return true;
    };
