...
- Overworld geometry (walk polygons, obstacles, interactable zones + hotspots, spawns) is data in `app/src/game/overworld/world.ts` over painted rooms. Two rects per station: the stand `zone` (small, center ON the walkable pad; prompts resolve by nearest zone center) and the `hotspot` (the prop's pixels; unwalkable clicks take the smallest containing rect, walkable clicks never resolve through hotspots). Occluder rects must hug the prop with `base` at its floor-contact line or they redraw floor over the player. Verify geometry by rendering it over the plate (the debug-overlay script pattern) plus a headless reachability flood; the in-scene backquote overlay shows the same live. Repaired-state art is baked onto the room texture at load (`room-<id>-live`); state patches live in `public/assets/overworld/states/`.
