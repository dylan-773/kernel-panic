...
2. If `vault/60-story/reveal-schedule.md` is stale relative to shipped content (new entries since last canonization), spawn `loremaster` to refresh it first - the Narrative Director paces against the ledger's knowledge table.
...
4. Spawn `loremaster` to gate. REVISE items go back to the Narrative Director with citations; max 2 rounds, contested items drop to the report. CHALLENGE verdicts (the doc looks wrong, including author-flagged `disputes`) go to the user, who rules before the item integrates.
...
7. If art orders were filed and budgeted, run the `/kp-art` play next. Close the cycle: archive `pipeline/BRIEF.md` to `pipeline/briefs/<id>.md`, mark item outcomes, append a dated line to `vault/00-index/revision-history.md`. Report (including every CHALLENGE and `suggestion` verbatim) and STOP; deploy only on explicit user OK.
