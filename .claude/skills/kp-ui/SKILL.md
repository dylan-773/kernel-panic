...
3. Gates, ONLY when they apply. A cycle that redesigns an EXISTING window is **pure UI** (RULINGS.md "Process", and the standing user ruling): no loremaster, no tutorial-agent, no game-code detours; gate-shaped concerns get a line in the demo's `NOTES.md` and one mention in the report. Gates run only when the cycle introduces NEW outward-facing fiction (a new window, new diegetic claims, new copy beyond relabeling) or NEW things the player must understand (a mechanic, stat, resource, or purchase surfacing for the first time). When they do run, spawn both in one parallel batch BEFORE the user sees the demo: `loremaster` ("is it true?") writes `pipeline/gates/loremaster-review.md`, `tutorial-agent` ("does the player know?") writes `pipeline/gates/tutorial-review.md`. REVISE and NEEDS-TEACHING go back to `ux-agent` with the citation, max 2 rounds; CHALLENGE verdicts (the doc looks wrong) go to the user, who rules. Gating before review is deliberate: what the user approves is what integrates, so a gate must never change a demo out from under an approval.
...
7. Report: items by type, orders filed, what the gates said (if they ran), every CHALLENGE and `suggestion` item verbatim, and the review URL `http://localhost:4180/kernel-panic-ui`. STOP. `kernel-panic-site/` is NOT touched in this mode. Run `/kp-art` next if art orders were filed and budgeted. (The brief archives at INTEGRATE time or when the user closes the cycle without integrating; a PROPOSE that ends at `awaiting` leaves the brief live.)
...
9. Close the cycle: archive `pipeline/BRIEF.md` to `pipeline/briefs/<id>.md`; if an integrated demo converts a panel from the RULINGS.md law-11 queue, update that queue's row IN THE SAME COMMIT (a queue that says "remaining" about shipped panels is how the spec rots); append a dated line to `vault/00-index/revision-history.md`.

10. Report: demos integrated, acceptance checks walked and their results, anything left `awaiting` and why. STOP; deploy only on explicit user OK.
