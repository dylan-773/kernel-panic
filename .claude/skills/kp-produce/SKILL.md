...
7. **Report and stop.** Summarize: items shipped per agent, items dropped and why, NEEDS-TEACHING items and how each was closed, final verdict + curve, credits spent. Then STOP. Deploy only on explicit user OK, via the standing flow (commit, push, deploy_website, live marker check).
